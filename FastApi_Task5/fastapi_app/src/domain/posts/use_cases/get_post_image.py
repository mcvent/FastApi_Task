...
class GetPostImageUseCase:
    ...
    async def execute(self, session: AsyncSession, post_id: int) -> FileResponse:
        try:
    ...
    async def get_all_images(self, session: AsyncSession, post_id: int) -> PostImagesListResponse:
        try:
            post = await self._post_repo.get_by_id(session, post_id)
            if not post:
                raise PostNotFoundByIdException(post_id)
    ...
    async def get_image_by_id(self, session: AsyncSession, post_id: int, image_id: int) -> FileResponse:
        try:
            post = await self._post_repo.get_by_id(session, post_id)
            if not post:
                raise PostNotFoundByIdException(post_id)
